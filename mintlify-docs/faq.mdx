...
Yes — natively on Windows 10 1809+ and Windows 11 (x64). PowerShell one-liner installer, NSIS setup wizard, or portable ZIP. Secrets are DPAPI-encrypted. WSL also works if you prefer the Linux path. See the [Windows guide](/windows) for limits (no embedded floating terminal, no Kitty image preview).
