...
Stuck on something? Drop into the [Discord](https://discord.gg/fX4H7GYSMJ) — fastest way to get an answer.

## Does it run on Windows?

Yes — natively on Windows 10 1809+ and Windows 11 (x64). Portable ZIP or NSIS setup wizard from the [latest release](https://github.com/proxysoul/soulforge/releases/latest). Secrets are DPAPI-encrypted. WSL also works if you prefer the Linux path. See [installation](/installation#windows-notes) for limits (no embedded floating terminal, no Kitty image preview).
...
Yes. Use it anywhere, anytime, for free — personal projects, startups, enterprise, client work, whatever. The [Business Source License 1.1](https://github.com/proxysoul/soulforge/blob/main/LICENSE) only kicks in if you want to *redistribute SoulForge as a commercial product* (repackage it, resell it, host it as a paid service). Converts to Apache 2.0 on March 15, 2030.

## What languages does code intelligence cover?

Tree-sitter detection covers: TypeScript, JavaScript, Python, Go, Rust, Java, Kotlin, Scala, C#, Swift, Dart, Elixir, OCaml, Lua, C, C++, Ruby, PHP, Zig, Bash, CSS, HTML, JSON, TOML, YAML, XML, Markdown, MDX, SQL, GraphQL, Protobuf, Dockerfile, Makefile, Nix, HCL/Terraform, Bazel/Starlark, Jsonnet, Vue, ReScript, Solidity, TLA+, Emacs Lisp, Objective-C.
